...
def compile_document(
    ...
    universal_skills = _apply_filters(
        skills_config["universal"], options.include_skills, options.exclude_skills
    )
    personas = _apply_filters(
        personas_list, options.include_personas, options.exclude_personas
    )
    ...
    for skill_name in universal_skills:
        skill_path = _find_file_in_locations(
            f"skills/{skill_name}/skill.md", personal_overlay, org_root, repo_root
        )
        if not skill_path:
    ...
    for lang, lang_skills in language_skills.items():
        ...
        for skill_name in lang_skills:
            skill_path = _find_file_in_locations(
                f"skills/{lang}/{skill_name}/skill.md",
                personal_overlay,
                org_root,
                repo_root,
            )
    ...
    if not options.no_personal and personal_overlay:
        ...
        # Personal skills (scan for skill folders containing skill.md)
        personal_skills_dir = personal_overlay / "skills"
        if personal_skills_dir.exists():
            for skill_dir in sorted(personal_skills_dir.iterdir()):
                if skill_dir.is_dir():
                    skill_file = skill_dir / "skill.md"
                    if skill_file.exists():
                        content = _read_file(skill_file)
                        sources.append(
                            (f"Personal Skill: {skill_dir.name}", skill_file, content)
                        )
        ...
        if personal_personas_dir.exists():
            for persona_file in sorted(personal_personas_dir.glob("*.md")):
                ...
                sources.append(
                    (f"Personal Persona: {persona_file.stem}", persona_file, content)
                )
...
def _build_traceability_header(
    metadata: Dict[str, Any], repo_root: Path, include_hash: bool
) -> str:
    ...
    if metadata.get("entry_point"):
        ...
        try:
            ...
            entry_point_rel = os.path.relpath(
                str(entry_point_path), str(repo_root_path)
            )
    ...
    if metadata.get("universal_skills"):
        lines.append(
            f"- **Universal Skills**: {', '.join(metadata['universal_skills'])}"
        )
    ...
    if metadata.get("project_rules"):
        ...
        try:
            ...
            project_rules_rel = os.path.relpath(
                str(project_rules_path), str(repo_root_path)
            )
