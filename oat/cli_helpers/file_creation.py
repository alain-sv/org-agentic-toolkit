...
def get_template_content(file_type: str, name: str = None, language: str = None) -> Tuple[str, bool]:
    """
...
def create_missing_file(
    ...
    if file_info.get("relative_path") is None:
        file_info.update(determine_file_type_and_path(
    ...
    if location == "PERSONAL":
        if not personal_overlay:
            error("Personal overlay not found. Cannot create file at PERSONAL level.", ctx)
            sys.exit(1)
