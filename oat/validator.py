...
def validate_repo(repo_root: Path, strict: bool = False) -> ValidationResult:
    ...
    if not agents_md_path.exists():
        result.add_warning(
            "AGENTS.md not found in repo root (recommended but not required)",
            agents_md_path,
        )
    ...
    if not constitution_path.exists():
        result.add_error(
            f"Constitution not found: {constitution_path}", constitution_path
        )
    ...
    for skill_name in universal_skills:
        skill_path = org_root / ".agent" / "skills" / skill_name / "skill.md"
    ...
    for lang, lang_skills in language_skills.items():
        for skill_name in lang_skills:
            skill_path = org_root / ".agent" / "skills" / lang / skill_name / "skill.md"
            if not skill_path.exists():
    ...
    if not project_md_path.exists():
        if strict:
            result.add_error(
                ".agent/project.md not found (required in strict mode)", project_md_path
            )
        else:
            result.add_warning(
                ".agent/project.md not found (recommended)", project_md_path
            )
...
def _validate_markdown_files(
    result: ValidationResult, org_root: Path, inherits_config: dict
):
        ...
        for skill_name in lang_skills:
            ...
            if skill_path.exists():
                try:
                    ...
                    result.add_error(
                        f"Language skill file not readable: {e}", skill_path
                    )
...
def _check_forbidden_constructs(
    result: ValidationResult, inherits_config: dict, inherits_path: Path
):
    ...
    if org_root and Path(org_root).is_absolute():
        result.add_error(
            "org_root must be a relative path (absolute paths are forbidden)",
            inherits_path,
        )
...
def validate_personal_overlay(
    personal_path: Path, strict: bool = False
) -> ValidationResult:
    """
    ...
    if not personal_path.exists():
        result.add_error(
            f"Personal overlay directory does not exist: {personal_path}", personal_path
        )
        return result
